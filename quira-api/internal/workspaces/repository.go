...
func (r *Repository) FindAll(limit, offset int, userId string) ([]Workspace, int, error) {
	query := "SELECT id, name, created_at, user_id FROM workspaces where workspaces.user_id = $3 order by created_at limit $1 offset $2"
...
func (r *Repository) Create(newWorkspace Workspace) (Workspace, error) {
	query := "INSERT INTO workspaces (name, user_id) VALUES (@name, @user_id) RETURNING id"
	args := pgx.NamedArgs{
